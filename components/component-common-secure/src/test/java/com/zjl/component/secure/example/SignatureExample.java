...
public class SignatureExample {
    ...
    public static void main(String[] args) {
        ...
        String sign2 = SignatureUtils.getSignRSA(INPUT_STR, privateKeyStringHex, EncType.BASE64,
                EncType.HEX);
        System.out.println("签名：" + sign2 + "\n length:" + sign2.length());
        System.out.println(
                "验证：" + SignatureUtils.verify(INPUT_STR, publicKeyString, sign2, EncType.BASE64,
                        EncType.BASE64));

        System.out.println("=====使用指定方法，指定加密key,默认编码,指定加签算法==================");
        String sign3 = SignatureUtils.getSignRSA(INPUT_STR, privateKeyString,
                SecureType.SignAlgorithm.SHA1WithRSA);
        System.out.println("签名：" + sign3 + "\n length:" + sign3.length());
        System.out.println("验证：" + SignatureUtils.verify(INPUT_STR, publicKeyString, sign3,
                SecureType.SignAlgorithm.SHA1WithRSA));

        System.out.println("=====使用指定方法，指定加密key,指定编码,指定加签算法==================");
        String sign4 = SignatureUtils.getSignRSA(INPUT_STR, privateKeyStringHex,
                SecureType.SignAlgorithm.SHA1WithRSA, EncType.BASE64, EncType.HEX);
        System.out.println("签名：" + sign4 + "\n length:" + sign4.length());
        System.out.println("验证：" + SignatureUtils.verify(INPUT_STR, publicKeyString, sign4,
                SecureType.SignAlgorithm.SHA1WithRSA, EncType.BASE64, EncType.BASE64));
    }
}
